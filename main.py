...
if __name__ == '__main__':
    model = models.alexnet(pretrained=True).eval().to("cuda")
    model.classifier = nn.Sequential(*[model.classifier[i] for i in range(6)])

    start = time.time()
    datasetReader = img_loader(r'C:\Users\fuchi\Desktop\Buffer')
    ...
    for i, (batch) in enumerate(dataloader):
        num_vec += len(batch[1])
        data_for_process = batch[0].to("cuda")
    ...
    with open(rf'C:\Users\fuchi\Desktop\image_vec_n_{num_vec}_d_{dim_vec}.csv', 'w', encoding='utf-8') as file:
